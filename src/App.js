...
function App() {
    ...
    return (
      semesters.map((semester) => (
        <>
          ...
          {checkSubjectCondition(semester, year.value) ? null : <td>
    ...
    return studentArray?.map((record) => {
      return (
        <>
          <tr>
            <td>
              <div className="student-items-flex">
              {record.value}
              </div>
            ...
            {record.columns.Year.map((colYear) => (
              <>
                ...
                {expandedYears.map((col) => col.value).includes(colYear.value) ? null : <td>
        ...
        <tr>
          <td>
            <div className="batch-items-flex">
              {expandedBatches.includes(record) ? <ArrowDropDownIcon onClick={() => { handleBatchClick(record) }} /> : <ArrowRightIcon onClick={() => { handleBatchClick(record) }} />}
  ...
  const renderSemesterColumns = (yearData) => {
    ...
    return (
      <th colSpan={semesterArray.length + filteredArray.length * 3 - filteredArray.length}>
        ...
        <div className="flex">{semesterArray.map((semval) =>
          <>
            ...
            {expandedSemesters.includes(semval) ? null : <th>
            ...
            {columndata?.map((coldata) => {
              return (
                <>
                  ...
                  {expandedYears.includes(coldata) ? null : <th>
                    <div className="columns-flex">
